...
var KisBpmChoseAssignmentCtrl = ['$scope', '$http', function($scope, $http) {
        ...
        $http({
            ...
            .success(function (data, status, headers, config) {
                ...
                for (var i=0; i<obj.length; i++) {
                    ...
                    roles.push({id:obj[i].roleId,name:obj[i].name});
                }
                $scope.roles = roles;
            })
    ...
    function getDetail(value,page,limit)
    {
        $http({
            ...
            headers: {'Accept': 'application/json',
                'Content-Type': 'application/x-www-form-urlencoded; charset=UTF-8'},
            url: '../mgr/listByRoleId?roleId='+value+"&page="+page+"&limit="+limit})
            .success(function (data, status, headers, config) {
                //封装数据
                var obj = data.data;
                if (data != null) {
                    var accounts = [];
                    for (var i=0; i<obj.length; i++) {
                        accounts.push({id:obj[i].userId, code : obj[i].name, name : obj[i].name, index:i});
                    }
                    $scope.accounts=accounts;
                }
                refreshPage(value,page,limit,data.count);
            })
            .error(function (data, status, headers, config) {
            });
    }
    ...
    $scope.getAllRoles = function (successCallback) {
        $http({
            ...
            .success(function (data, status, headers, config) {
                var obj = data.data;
                for (var i=0; i<obj.length; i++) {
                    candidateGroups.push({id:obj[i].roleId,name:obj[i].name,description:obj[i].description});
                }
